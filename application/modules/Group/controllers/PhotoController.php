...
class Group_PhotoController extends Core_Controller_Action_Standard
{
    public function init()
    {
        if (!Engine_Api::_()->core()->hasSubject()) {
            if (0 !== ($photo_id = (int)$this->_getParam('photo_id')) && null !== ($photo =
                Engine_Api::_()->getItem('group_photo', $photo_id))) {
                Engine_Api::_()->core()->setSubject($photo);
            } else
                if (0 !== ($group_id = (int)$this->_getParam('group_id')) && null !== ($group =
                    Engine_Api::_()->getItem('group', $group_id))) {
                    Engine_Api::_()->core()->setSubject($group);
                }
        }

        $this->_helper->requireUser->addActionRequires(array('upload', 'upload-photo',
            // Not sure if this is the right
            'edit', ));

        $this->_helper->requireSubject->setActionRequireTypes(array('list' => 'group',
            'upload' => 'group', 'view' => 'group_photo', 'edit' => 'group_photo', ));
    }
    ...
    public function viewAction()
    {
        ...
        if (!$viewer || !$viewer->getIdentity() || $photo->user_id != $viewer->
            getIdentity()) {
    ...
    public function uploadAction()
    {
        ...
        if (!$this->_helper->requireAuth()->setAuthParams($group, null, 'photo')->
            isValid())
            return;
        ...
        try {
            ...
            $params = array('group_id' => $group->getIdentity(), 'user_id' => $viewer->
                getIdentity(), );

            // Add action and attachments
            $api = Engine_Api::_()->getDbtable('actions', 'activity');
            $action = $api->addActivity(Engine_Api::_()->user()->getViewer(), $group,
                'group_photo_upload', null, array('count' => count($values['file'])));
    ...
    public function uploadPhotoAction()
    {
        ...
        try {
            ...
            $params = array( // We can set them now since only one album is allowed
                ...
                'group_id' => $group->getIdentity(), 'user_id' => $viewer->getIdentity(), );
            $photo_id = Engine_Api::_()->group()->createPhoto($params, $_FILES['Filedata'])->
                photo_id;
    ...
    public function editAction()
    {
        ...
        return $this->_forward('success', 'utility', 'core', array('messages' => array(Zend_Registry::
            get('Zend_Translate')->_('Changes saved')), 'layout' => 'default-simple',
            'parentRefresh' => true, 'smoothboxClose' => true));
    }
    ...
    public function deleteAction()
    {
        ...
        return $this->_forward('success', 'utility', 'core', array('messages' => array(Zend_Registry::
            get('Zend_Translate')->_('Photo deleted')), 'layout' => 'default-simple',
            'parentRedirect' => $group->getHref(), 'smoothboxClose' => true, ));
    }
}
