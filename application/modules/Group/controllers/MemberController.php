...
class Group_MemberController extends Core_Controller_Action_Standard
{
  ...
  Deinvite member 
  */
  public function deinviteAction(){
		// Check auth
		if( !$this->_helper->requireUser()->isValid() ) return;
		if( !$this->_helper->requireSubject()->isValid() ) return;

		// Make form
		$this->view->form = $form = new Group_Form_Member_Deinvite();

		// Process form
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			$viewer = $this->_helper->api()->user()->getViewer();
			$id_membership= $this->getRequest()->getParam('user_id');
			$subject = $this->_helper->api()->core()->getSubject();
			...
			try
			{
				$subject->membership()->deInvite($viewer, $id_membership);
				$notifyApi = Engine_Api::_()->getDbtable('notifications', 'activity');
				$select = $notifyApi->select()
			        ->where('object_id = ?', $subject->getIdentity())
			        ->where('user_id = ?', $id_membership)
			        ->where('type =?', 'group_invite');
			
			    $select = $select->limit(1);
			    $row = $notifyApi->fetchRow($select);
			    $row->delete();
				$db->commit();
			}
			catch( Exception $e )
			...
			return $this->_forward('success', 'utility', 'core', array(
		        'messages' => array(Zend_Registry::get('Zend_Translate')->_('Cancel Invite Success.')),
		        'layout' => 'default-simple',
		      	'smoothboxClose'=> true,
		        'parentRefresh' => true,
			));
