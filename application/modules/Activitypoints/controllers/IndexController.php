...
class Activitypoints_IndexController extends Core_Controller_Action_Standard
{
  ...
  public function helpAction()
  {
    ...
    while($row = $statement->fetch()) {
	  ...
	  if(empty($row['type']) && (($row['action_custom'] == 0) || (($row['action_custom'] == 1) && !empty($row['action_module']) && !Engine_Api::_()->getDbTable('modules','core')->hasModule($row['action_module'])))) {
		continue;
	  }
  ...
  public function sendpointsAction()
  {
	...
	if($result['is_error'] == 1) {
	  ...
	  $response['status'] = 0;
	  $response['msg'] = $result['message'];
	  $response['balance'] = $api->getPointsBalance( $viewer->getIdentity() );
	}
