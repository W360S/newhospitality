...
<div class="pt-block" style="margin-top:18px">
    <div class="pt-contents-response">
		
			<p><?php echo $this->translate("Get spotted by your dream employer!");?></p>			
            <a style="color: #fff;font-size: 14px;text-decoration: none;text-transform: uppercase;"  href="<?php echo $this->baseUrl().PATH_SERVER_INDEX.'/resumes/index/resume-info' ?>"><?php echo $this->translate("Create a Resume ")?>(<?php $re= 3- count($this->sum_resume); echo $re. $this->translate(" Left"); ?>)</a>
...
function view_resume(){
    ...
    if(value != 0){
        window.location.href= "<?php echo $this->baseUrl().PATH_SERVER_INDEX.'/resumes/index/preview/resume_id/'?>"+value;
    }
}
</script>
