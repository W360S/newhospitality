    ...
    <tbody>
      <?php foreach ($this->paginator as $item): ?>
        <tr>
          ...
          <td><?php echo $this->htmlLink(array('route' => 'view-school-artical', 'id' => $item->artical_id), wordwrap($item->title, "64", "<br />\n", true)) ?></td>
          <td><?php echo $this->school($item->school_id)->name;?></td>
          <td><?php echo $this->user($item->user_id);?></td>
          
          <td><?php echo date('d F Y', strtotime($item->created)) ?></td>
          <td>
           <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'school', 'controller' => 'admin-artical', 'action' => 'edit', 'artical_id' => $item->artical_id), $this->translate('edit')
            ) ?>
            |
            <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'school', 'controller' => 'admin-artical', 'action' => 'delete', 'id' => $item->artical_id), $this->translate('delete'), array(
              'class' => 'smoothbox',
            )) ?>
            <?php if($item->approved !=1){?>
            |
            <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'school', 'controller' => 'admin-artical', 'action' => 'approve', 'id' => $item->artical_id), $this->translate('approve'), array(
              'class' => 'smoothbox',
            )) ?>
            <?php } ?>
          </td>
...
<?php else: ?>
  <div class="tip">
    <span>
      <?php echo $this->translate("There are no articals posted by members yet.") ?>
    </span>
  </div>
<?php endif; ?>
