            ...
            try {
                foreach($arr_send_mail_users as $item){
                   ...
                   $defaultParams = array(
                    ...
                    Engine_Api::_()->getApi('mail', 'core')->sendSystem($manage_job,
                    'send_mail_manage_job', $defaultParams);
                }
    ...
    if(count($row)>0){
        ...
        if ($this->_request->isPost())
        {
          ...
          if(!empty($title) && $check_file){
              ...
              // Send
              try {
                Engine_Api::_()->getApi('mail', 'core')->sendSystem($to_user,
                    'apply_job', $defaultParams);
              } catch( Exception $e ) {
                // Silence exception
              }
