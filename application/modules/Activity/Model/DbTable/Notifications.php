...
class Activity_Model_DbTable_Notifications extends Engine_Db_Table {
    ...
    /** quangnvh
     * Get a paginator for requests-friend notifications
     * 
     * @param User_Model_User $user
     * @return Zend_Paginator
     */
    public function getCountFriendRequest(User_Model_User $user) {
        // Only get enabled types
        $enabledModuleNames = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
        ...
        $select = $this->select()
                ...
                ->where('module IN(?)', $enabledModuleNames)
                ->where('user_id = ?', $user->getIdentity())
                ->where('engine4_activity_notificationtypes.type = ?', 'friend_request')
                ->where('is_request = ?', 1)
                ->where('mitigated = ?', 0)
                ->order('date ASC')
        ;
        $data = $typeTable->fetchAll($select);
        return count($data);
    }

    public function getRequestFriendPaginator(User_Model_User $user) {
        // Only get enabled types
        $enabledModuleNames = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
