...
class Storage_Model_DbTable_Files extends Engine_Db_Table {
    ...
    public function listPhotoFriends($photo_id) {
        if ($photo_id) {
            $select = $this->select()
                    ->where('parent_file_id = ?', $photo_id)
                    //->where('parent_file_id= ?', $photo_id)
                    ->where('type = ?', 'thumb.icon');

            $file = $this->fetchRow($select);
            //Zend_Debug::dump($file, 'file');exit();
            return $file;
        }
    }

    public function listPhotoVideos($photo_id) {
        if ($photo_id) {
            $select = $this->select()
    ...
    // quangnvh
    public function listAllPhoto($photo_id) {
        if ($photo_id) {
            $select = $this->select()
                    ->where('file_id = ? or parent_file_id= ?', $photo_id, $photo_id);
