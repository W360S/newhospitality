 ...
 * @version    $Id: classified.tpl (var) 2010-08-17 9:40:21Z SocialEngineAddOns $
...
$suggestion_viewfriend_users_array = $this->classified_combind_path;
$suggestion_message = count($this->classified_combind_path);
$suggestion_viewfriend_users_str = implode(",", $suggestion_viewfriend_users_array);
?>

<script type="text/javascript">
	var classified_displayed_suggestions = "<?php echo $suggestion_viewfriend_users_str; ?>";
	var classified_sugg_message = "<?php echo $suggestion_message; ?>";
	var action_session_id = "<?php echo $this->classified_id;  ?>";
	...
	var action_module = 'classified';
	// Note : In the classified suggestions popup, the baseurl variable of JS does not get set, because default.tpl is not called for this. Thus, we set it explicitly.
	en4.core.setBaseUrl('<?php echo $this->url(array(), 'default', true) ?>');
	
	en4.suggestion.classifieds = {
		
		classifiedUser : function(classified_div_id)
		{
		  var request = new Request.JSON({   	
		    url : en4.core.baseUrl + 'suggestion/main/suggestion-classified',
		    data : {
		      format : 'json',    
		      classified_div_id : classified_div_id,
		      session_classified_id : action_session_id,
		      classified_dis_sugg : classified_displayed_suggestions	      
		    }
		  });
		  request.send();
		  return request;
...
<div class="suggestion_popup">
	<?php $classified_displayname = Engine_Api::_()->getItem('classified', $this->classified_id)->title;  ?>
  <h4><?php echo $this->translate("Select friends of yours who might be interested in ") . '<u>' . $classified_displayname . '</u>'; ?></h4>
		<p><?php echo $this->translate("Selected friends will get a suggestion from you to view this classified."); ?></p>
	<div class="suggestion_member_search_box">
		<div class="classified_members_search" style="float:left;">
      <input id="classified_members_search_inputd" type="text" value="<?php echo $this->translate("Search Members"); ?>"  onkeyup="show_searched_friends(0, event);" >
    </div>
    <div class="link">
...
<?php } ?>
	...
	<form name="suggestion" method="POST" class="suggestion_form_popup" >
		<input type="hidden" name="classified" value="<?php echo $this->classified_id;  ?>" />
		<div id="hidden_checkbox"> </div>
		<div class="suggestion_popup_box">	
			<?php $div_id = 1;
			$send_request_user_info_array = array();
		if (!empty($this->classified_combind_path)) {
		...
		<!--<div id="new_classified_show" class="suggestion_seleted_box"> </div>-->
		<?php if( $this->members->count() > 1 ): ?>
	    <div class="pagination">
	      <?php if( $this->members->getCurrentPageNumber() > 1 ): ?>
	        <div id="user_classified_members_previous" class="paginator_previous" style="font-weight:bold;">
	      ...
	      <?php if( $this->members->getCurrentPageNumber() < $this->members->count() ): ?>
	        <div id="user_classified_members_next" class="paginator_next" style="font-weight:bold;">
