 ...
 * @version    $Id: group.tpl (var) 2010-08-17 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */


//MAKE STRING OF CURRENT USER DISPLAY
$suggestion_viewfriend_users_array = $this->group_combind_path;
$suggestion_message = count($this->group_combind_path);
$suggestion_viewfriend_users_str = implode(",", $suggestion_viewfriend_users_array);
?>

<script type="text/javascript">
	var suggestion_string = '';
	var group_friend_id_str = '';
	...
	var group_displayed_suggestions = "<?php echo $suggestion_viewfriend_users_str; ?>";
	var memberSearch = '<?php echo $this->search ?>';
	var memberPage = <?php echo sprintf('%d', $this->members->getCurrentPageNumber()) ?>;
	var grp_sugg_message = "<?php echo $suggestion_message; ?>";
	var action_session_id = "<?php echo $this->group_id;  ?>";
	var action_module = 'group';
	
	
	// Note : In the group suggestions popup, the baseurl variable of JS does not get set, because default.tpl is not called for this. Thus, we set it explicitly.
	en4.core.setBaseUrl('<?php echo $this->url(array(), 'default', true) ?>');
	
	en4.suggestion.groups = {
		
		groupUser : function(group_div_id)
		{
		  var request = new Request.JSON({   	
		    url : en4.core.baseUrl + 'suggestion/main/suggestion-group',
		    data : {
		      format : 'json',    
		      group_div_id : group_div_id,
		      session_group_id : action_session_id,
		      group_dis_sugg : group_displayed_suggestions	      
		    }
		  });
		  request.send();
		  return request;
...
<div class="suggestion_popup">
	<?php $group_displayname = Engine_Api::_()->getItem('group', $this->group_id)->title;  ?>
    <?php $url= $this->baseUrl().'/group/'.Engine_Api::_()->getItem('group', $this->group_id)->group_id;?>
	<h4><?php echo $this->translate("Select friends of yours who might be interested in joining ") . "<a href='$url' target='_parent' >" . $group_displayname . '</a>'; ?></h4>
	<p><?php echo $this->translate("Selected friends will get a suggestion from you to join this group."); ?></p>
	<div class="suggestion_member_search_box">
		...
		<div class="group_members_search" style="float:left;">
            <input id="group_members_search_inputd" type="text" value="<?php echo $this->translate("Search Members"); ?>"  onkeyup="show_searched_friends(0, event);">
        </div>
        <div class="link">
...
<?php } ?>
	...
	<form name="suggestion" method="POST" class="suggestion_form_popup" >
		<input type="hidden" name="group" value="<?php echo $this->group_id;  ?>" />
		...
		if (!empty($this->group_combind_path)) {
		...
		<div id="new_group_show" class="suggestion_seleted_box"> </div>
		<?php if( $this->members->count() > 1 ): ?>
	    <div class="pagination">
	      <?php if( $this->members->getCurrentPageNumber() > 1 ): ?>
	        <div id="user_group_members_previous" class="paginator_previous" style="font-weight:bold;">
	      ...
	      <?php if( $this->members->getCurrentPageNumber() < $this->members->count() ): ?>
	        <div id="user_group_members_next" class="paginator_next" style="font-weight:bold;">
