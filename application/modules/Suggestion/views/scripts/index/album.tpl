 ...
 * @version    $Id: album.tpl (var) 2010-08-17 9:40:21Z SocialEngineAddOns $
...
$suggestion_viewfriend_users_array = $this->album_combind_path;
$suggestion_message = count($this->album_combind_path);
$suggestion_viewfriend_users_str = implode(",", $suggestion_viewfriend_users_array);
?>

<script type="text/javascript">
	var album_displayed_suggestions = "<?php echo $suggestion_viewfriend_users_str; ?>";
	var album_sugg_message = "<?php echo $suggestion_message; ?>";
	var action_session_id = "<?php echo $this->album_id;  ?>";
	...
	var action_module = 'album';
	// Note : In the album suggestions popup, the baseurl variable of JS does not get set, because default.tpl is not called for this. Thus, we set it explicitly.
	en4.core.setBaseUrl('<?php echo $this->url(array(), 'default', true) ?>');
	
	en4.suggestion.albums = {
		
		albumUser : function(album_div_id)
		{
		  var request = new Request.JSON({   	
		    url : en4.core.baseUrl + 'suggestion/main/suggestion-album',
		    data : {
		      format : 'json',    
		      album_div_id : album_div_id,
		      session_album_id : action_session_id,
		      album_dis_sugg : album_displayed_suggestions	      
		    }
		  });
		  request.send();
		  return request;
...
<div class="suggestion_popup">
	<?php $album_displayname = Engine_Api::_()->getItem('album', $this->album_id)->title;  ?>
  <h4><?php echo $this->translate("Select friends of yours who might be interested in ") . '<u>' . $album_displayname . '</u>'; ?></h4>
		<p><?php echo $this->translate("Selected friends will get a suggestion from you to view this album."); ?></p>
	<div class="suggestion_member_search_box">
		...
		<div class="album_members_search" style="float:left;">
      <input id="album_members_search_inputd" type="text" value="<?php echo $this->translate("Search Members"); ?>" onkeyup="show_searched_friends(0, event);">
    </div>
    <div class="link">
...
<?php } ?>
	...
	<form name="suggestion" method="POST" class="suggestion_form_popup" >
		<input type="hidden" name="album" value="<?php echo $this->album_id;  ?>" />
		...
		if (!empty($this->album_combind_path)) {
		...
		<!--<div id="new_album_show" class="suggestion_seleted_box"> </div>-->
		<?php if( $this->members->count() > 1 ): ?>
	    <div class="pagination">
	      <?php if( $this->members->getCurrentPageNumber() > 1 ): ?>
	        <div id="user_album_members_previous" class="paginator_previous" style="font-weight:bold;">
	      ...
	      <?php if( $this->members->getCurrentPageNumber() < $this->members->count() ): ?>
	        <div id="user_album_members_next" class="paginator_next" style="font-weight:bold;">
