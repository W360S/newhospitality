 ...
 * @version    $Id: music.tpl (var) 2010-08-17 9:40:21Z SocialEngineAddOns $
...
$suggestion_viewfriend_users_array = $this->music_combind_path;
$suggestion_message = count($this->music_combind_path);
$suggestion_viewfriend_users_str = implode(",", $suggestion_viewfriend_users_array);
?>

<script type="text/javascript">
	var music_displayed_suggestions = "<?php echo $suggestion_viewfriend_users_str; ?>";
	var music_sugg_message = "<?php echo $suggestion_message; ?>";
	var action_session_id = "<?php echo $this->music_id;  ?>";
	...
	var action_module = 'music';
	// Note : In the music suggestions popup, the baseurl variable of JS does not get set, because default.tpl is not called for this. Thus, we set it explicitly.
	en4.core.setBaseUrl('<?php echo $this->url(array(), 'default', true) ?>');
	
	en4.suggestion.musics = {
		
		musicUser : function(music_div_id)
		{
		  var request = new Request.JSON({   	
		    url : en4.core.baseUrl + 'suggestion/main/suggestion-music',
		    data : {
		      format : 'json',    
		      music_div_id : music_div_id,
		      session_music_id : action_session_id,
		      music_dis_sugg : music_displayed_suggestions	      
		    }
		  });
		  request.send();
		  return request;
...
<div class="suggestion_popup">
	<?php $music_displayname = Engine_Api::_()->getItem('music_playlist', $this->music_id)->title;  ?>
		<h4><?php echo $this->translate("Select friends of yours who might be interested in ") . '<u>' . $music_displayname . '</u>'; ?></h4>
		<p><?php echo $this->translate("Selected friends will get a suggestion from you to listen to this music."); ?></p>
	<div class="suggestion_member_search_box">
		...
		<div class="music_members_search" style="float:left;">
      <input id="music_members_search_inputd" type="text" value="<?php echo $this->translate("Search Members"); ?>" onkeyup="show_searched_friends(0, event);">
    </div>
    <div class="link">
...
<?php }  ?>
	...
	<form name="suggestion" method="POST" class="suggestion_form_popup" >
		<input type="hidden" name="music" value="<?php echo $this->music_id;  ?>" />
		...
		if (!empty($this->music_combind_path)) {
		...
		<!--<div id="new_music_show" class="suggestion_seleted_box"> </div>-->
		<?php if( $this->members->count() > 1 ): ?>
	    <div class="pagination">
	      <?php if( $this->members->getCurrentPageNumber() > 1 ): ?>
	        <div id="user_music_members_previous" class="paginator_previous" style="font-weight:bold;">
	      ...
	      <?php if( $this->members->getCurrentPageNumber() < $this->members->count() ): ?>
	        <div id="user_music_members_next" class="paginator_next" style="font-weight:bold;">
