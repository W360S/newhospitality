...
{ 
	switch ($row_mix_key)
	{
		...
		case 'friend':?>
					...
					{ ?>
						<script type="text/javascript">
						var add_id = "<?php echo 'friend_' . $row_mix_value[0]->user_id; ?>";
						// Suggestion which are "Disply" in the widget.
						display_sugg += ',' + add_id;
						</script>						
							...
							<div class="suggestion_list">						
									...
									<div class="item_details">
											<?php echo	'<a style="margin-top:5px;" class="suggest_cancel" title="' . $this->translate('Do not show this suggestion') . '" href="javascript:void(0);" onclick="mixInfo(' . $row_mix_value[0]->user_id . ', \''.$this->div_id.'\', \'friend\', \'' . $this->page_name . '\');"></a>';?>
									<b><?php
										if($this->page_name == 'new')
										{
											echo $this->htmlLink($row_mix_value[0]->getHref(), $row_mix_value[0]->getTitle(), array('title' => $row_mix_value[0]->getTitle()));
										 ...
										 ?></b>										 
											 <?php
											 if($this->page_name == 'mix' || $this->page_name == 'friend')
											 {
											 		if(!empty($this->mutual_friend_array[$row_mix_value[0]->user_id]))
											 		{
											 	  	echo '<div><a class="smoothbox" href="' . $this->url(array('module' => 'suggestion', 'controller' => 'index', 'action' => 'mutualfriend', 'sugg_friend_id' => $row_mix_value[0]->user_id), 'default', true) . '">' . $this->translate(array('%s mutual friend', '%s mutual friends', $this->mutual_friend_array[$row_mix_value[0]->user_id]),$this->locale()->toNumber($this->mutual_friend_array[$row_mix_value[0]->user_id])) . '</a></div>';
											 		}
											 }
											 ?>
			...
			<div style="display:none">
				<form enctype="application/x-www-form-urlencoded" id="ajax-add-friend-<?php echo $row_mix_value[0]->user_id; ?>" action="/members/friends/add/user_id/<?php echo $row_mix_value[0]->user_id; ?>">
				</form>
			</div>
			<?php }
			// If no suggestion available.
