  ...
  public function detailAction(){
    ...
    ->from($questionsCatName, 
		...
		GROUP_CONCAT(Distinct(engine4_experts_categories.category_name) SEPARATOR ', ') as category, 
                            ...
                            (SELECT 
                                GROUP_CONCAT(
                                    CONCAT((SELECT displayname FROM engine4_users where user_id = engine4_experts_likes.user_id))
                                     SEPARATOR '&&'
                                )
                             FROM engine4_experts_likes WHERE engine4_experts_likes.question_id = engine4_experts_questionscategories.question_id
                            ) as like_name,
                            engine4_experts_questionscategories.*, 
                            engine4_experts_questions.*,
