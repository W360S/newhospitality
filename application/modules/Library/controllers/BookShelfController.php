...
class Library_BookShelfController extends Core_Controller_Action_Standard {
    ...
    public function downloadAction() {
        ...
        $result = Engine_Api::_()->library()->addBookShelf($book_credit->book_id, $user_id);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($bookshelf->downloaded == 1 || $book_credit['credit'] == 0) {
                ...
                if (file_exists($file)) {
                    //Zend_Debug::dump($bookshelf); 
                    //print_r($bookshelf);
                    //print_r($book_credit);
                       
                    $bookshelf_select = Engine_Api::_()->getDbtable('bookshelfs', 'library')
                    ->select()
                    ->where('latest_download > ?', date('Y-m-d',strtotime("-1 days")))
                    ->where('user_id = ?', $user_id);
                    $latest_downloads = Engine_Api::_()->getDbtable('bookshelfs', 'library')->fetchAll($bookshelf_select);
                    
                    if(count($latest_downloads) > 2){
                        $response->setBody('Sorry, you can not download 2 freebook a day');
                    }else{
                        ...
                        $bookshelf->latest_download = date('Y-m-d');
                        $bookshelf->save();
